function pretestCPLEX(
    ...
    JuMP.@variable(m, y[i = 1:n_items, g = 1:nCouples], Bin)
    #length
    ...
    for g = 1:size(alphas, 1)
        JuMP.@constraint(
            m,
            [t = 1:T_pt],
            sum(x[i, t] for i in findall(guessedDifficulty .== g)) <= alphas[g] * n_pt + 1
        )
    end
    #item use min=1 use all items
    JuMP.@constraint(m, [i = 1:n_items], sum(x[i, t] for t = 1:T_pt) >= 1)
    #overlap, 5 btw each pair
    JuMP.@constraint(m, [g = 1:nCouples], sum(y[i, g] for i = 1:n_items) == ol_max[g])
    JuMP.@constraint(
        m,
        [g = 1:nCouples, i = 1:n_items],
...
function overlapPairs(T_pt, opMatrix)
    ...
    f1 = [couples[g][1] for g = 1:nCouples]
    f2 = [couples[g][2] for g = 1:nCouples]
    for g = 1:nCouples
